...
try {
  ...
  const transformed = raw.map((item, index) => {
    ...
    const price = getSafe(item, 'Price') || '$' + (Math.floor(Math.random() * 800) + 200); // randomize if missing
    const imgURL = getSafe(item, 'Image') || `https://picsum.photos/seed/phone${index}/800/600`;
    ...
    return {
      ...
      aiInsights: "This phone is part of the comprehensive mobile dataset. " + (item['Model Name'] || ''),
      rating: parseFloat((Math.random() * 1.5 + 3.5).toFixed(1)) // randomize between 3.5 and 5.0
    };
  });

  fs.writeFileSync('./src/mobile_data_transformed.json', JSON.stringify(transformed, null, 2));
  console.log('Successfully transformed ' + transformed.length + ' phones.');
} catch(e) {
